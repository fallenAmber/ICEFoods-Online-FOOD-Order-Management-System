            ...
            <table class="table table-hover">
                ...
                <tbody>
                    ...
                    foreach ($order_items as $item) {
                        $sql = "SELECT * FROM foods WHERE id = " . $item['id'];
                        $product = $db->query($sql)->fetch();
                        $total += $item['quantity'] * $product['price'];
                        echo '<tr>';
                        echo '<td><img src="/assets/images/foods/' . $product['img'] . '" alt="' . $product['name'] . '" class="img-fluid" width="50" height="50">' . $product['name'] . '</td>';
                        echo '<td>' . $product['price'] . ' Taka</td>';
                        echo '<td>' . $item['quantity'] . '</td>';
                        echo '<td>' . $item['quantity'] * $product['price'] . ' Taka</td>';
                        echo '</tr>';
                    }
                    ?>
                </tbody>
